...
def print_split_summary(name, frame):
    source_counts = frame["image"].apply(lambda x: "2015" if "_" in str(x) else "2019").value_counts().to_dict()
    class_counts = frame["level"].value_counts().sort_index().to_dict()
    print(f"{name} source mix: {source_counts}")
    print(f"{name} class mix: {class_counts}")
...
def train(
    ...
    # Step B: Patient-aware validation split for 2015 Data (Anti-Data Leakage + better class balance)
    df_2015['patient_id'] = df_2015['image'].apply(lambda x: str(x).split('_')[0])
    if StratifiedGroupKFold is not None and df_2015['patient_id'].nunique() >= 5:
        sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42)
        t_idx_15, v_idx_15 = next(sgkf.split(df_2015, df_2015['level'], groups=df_2015['patient_id']))
